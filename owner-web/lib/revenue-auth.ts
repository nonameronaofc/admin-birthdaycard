...
const REVENUE_COOKIE = 'owner_revenue';

function getPassword() {
  return process.env.OWNER_REVENUE_PASSWORD || process.env.ADMIN_REVENUE_PASSWORD || '';
}
...
function tokenForPassword(password: string) {
  return sha256Hex(`owner-revenue:${password}`);
}
...
export function setRevenueCookie() {
  ...
  if (!password) throw new Error('OWNER_REVENUE_PASSWORD belum diatur.');
